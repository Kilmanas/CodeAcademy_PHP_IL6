...
class Messages extends AbstractModel implements ModelInterface
{
    ...
    {
        $db = new DBHelper();
        $message = $db->select()->from(self::TABLE)->where('id', (string)$id)->getOne();
        $this->id = (int)$message['id'];
        $this->userFrom = (int)$message['user_from'];
        $this->userTo = (int)$message['user_to'];
    ...
    {
        $db = new DBHelper();
        $data = $db->select()->from(self::TABLE)->where('user_to', (string)$userId)->andWhere('opened', (string)0)->get();
        $messages = [];
    ...
    {
        $db = new DBHelper();
        $data = $db->select("id")->from("users")->where("id", (string) $this->userFrom)->getOne();
        $user = new User();
    ...
    public static function getChatMessages(int $fromId, int $toId): array
    {
        ...
        $data = $db->select()
            ->from(self::TABLE)
            ->where('user_from', (string) $fromId)
            ->andWhere('user_to', (string) $toId)
            ->get();
        $messages = [];
        foreach ($data as $element) {
            $message = new Messages();
    ...
    public static function getUserMessages()
    {
        ...
        $data = $db->select()->from(self::TABLE)->where('user_from', (string) $userId)->orWhere('user_to', (string) $userId)->get();
        $messages = [];
            foreach ($data as $element) {
               $message = new Messages();
    ...
    public static function getMessagesWithFriend($friendId)
    {
        ...
        $data = $db->select()
            ->from(self::TABLE)
            ->where('user_from', (string) $userId)
            ->andWhere('user_to',(string) $friendId)
            ->orWhere('user_from',(string) $friendId)
            ->andWhere('user_to',(string) $userId)
            ->get();
        $messages = [];
        foreach ($data as $element) {
            $message = new Messages();
    ...
    {
        $db = new DBHelper();
        $db->update(self::TABLE, ['opened' => 1])
            ->where('user_from', (string) $senderId)
            ->andWhere('user_to', (string) $receiverId)
            ->andWhere('opened', (string) 0)
            ->exec();
    }
}